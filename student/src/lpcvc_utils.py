...
def format_competition_json(step2_draft: Dict[str, object]) -> Dict[str, object]:
    per_criterion = []
    any_positive = False
    for criterion in CRITERIA:
...
def consistency_target_from_trace(trace: Dict[str, object]) -> Dict[str, object]:
    ...
    for item in trace.get("per_criterion", []):
        ...
        consistent = not ((score == 1 and (evidence == DEFAULT_EVIDENCE or has_negative_marker or support_type == "unsupported")))
